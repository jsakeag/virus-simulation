...
# body1.add_velocity(np.array([[200, 0, 0]]))
...
# body3.add_velocity(np.array([[250, 0, 0]]))

body_list = []
initial_infected = 0
for i in range(15):
    # generate random bool to determine isInfected
    isInfected = bool(random.getrandbits(1))

    body = Body(np.array([[random.randrange(0, 1000), random.randrange(
        0, 800), 0]]), 6 * pow(10, 15), (100, 100, 100), 20)
    body.add_velocity(np.array(
        [[random.randrange(-200, 200, 399), random.randrange(-200, 200, 399), 0]]))
    if(isInfected):
        body.infect()
        initial_infected += 1
...
print("//////////////////////////////////////////////////////////////////////")
print("Initial infected: " + str(initial_infected))
sim = Simulation()
sim.initialise_environment(body_list, initial_infected)
